{@type:sensor}
input:Length(3, "拉尾盤日數");
input:Ratio(1, "拉尾盤幅度%");
input:closetime(132500, "尾盤前時間");
input:ratiotoLow(7, "低檔起算漲幅%");//距離區間最低價多少%
input:daystoLow(5, "距離最低價天數");//輸入區間最低價的區間是幾天
input:TXT1("最高算5天", "拉尾盤日數使用限制");
input:TXT2("限用5分鐘", "頻率限制");
